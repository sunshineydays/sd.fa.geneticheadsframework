    ...
    public static class Patch_MouthControllerComp
    {
        ...
        static bool Prefix(object __instance)
        {
            try
            {
                ...
                if (fi.FieldType != typeof(MouthTypeDef)) // e.g. EyeballTypeDef
                {
                   //Log.Warning($"[FA GenePatch] faceType field is not of type {typeof(MouthTypeDef).Name}");
                    return true;
                }
                ...
                if (chosen != null)
                {
                    ...
                    if (Prefs.DevMode)
                        Log.Message($"[FA GenePatch] {typeof(MouthTypeDef).Name}: assigned {chosen.defName} to {pawn.LabelShortCap}");
                }
