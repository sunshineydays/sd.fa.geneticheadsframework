    ...
    public static class Patch_SkinControllerComp
    {
        ...
        static bool Prefix(object __instance)
        {
            try
            {
                ...
                if (fi.FieldType != typeof(SkinTypeDef)) // e.g. EyeballTypeDef
                {
                   //Log.Warning($"[FA GenePatch] faceType field is not of type {typeof(SkinTypeDef).Name}");
                    return true;
                }
                ...
                if (chosen != null)
                {
                    ...
                    if (Prefs.DevMode)
                        Log.Message($"[FA GenePatch] {typeof(SkinTypeDef).Name}: assigned {chosen.defName} to {pawn.LabelShortCap}");
                }
