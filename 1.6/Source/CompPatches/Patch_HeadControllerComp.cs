...
namespace FacialAnimationGeneticHeads.CompPatches
{
    ...
    public static class Patch_HeadControllerComp
    {
        static MethodBase TargetMethod() =>
            AccessTools.Method(AccessTools.TypeByName("FacialAnimation.HeadControllerComp"), "InitializeIfNeed");
        ...
        static bool Prefix(object __instance)
        {
            try
            {
                ...
                if (fi.FieldType != typeof(HeadTypeDef)) // e.g. EyeballTypeDef
                {
                   //Log.Warning($"[FA GenePatch] faceType field is not of type {typeof(HeadTypeDef).Name}");
                    return true;
                }

                var current = fi.GetValue(__instance) as HeadTypeDef;
                if (current != null) return true;

                var chosen = GeneFacePatchHelper.GetGeneMatchedDef<HeadTypeDef>(pawn, pawn.gender);
                if (chosen != null)
                {
                    fi.SetValue(__instance, chosen);
                    if (Prefs.DevMode)
                        Log.Message($"[FA GenePatch] {typeof(HeadTypeDef).Name}: assigned {chosen.defName} to {pawn.LabelShortCap}");
                }
